// https://github.com/Gezine/BD-UN-JB/blob/main/payloads/poops/src/org/bdj/external/Poops.java
...
typedef struct {
    ...
    volatile s32 finished;
    volatile s32 command;   
} worker_state_t;
...
static void worker_init(worker_state_t *ws, s32 total)
{
    ws->total   = total;
    ws->started = 0;
    ws->finished= 0;
...
static s32 worker_wait_for_work(worker_state_t *ws)
{
    ...
    __sync_fetch_and_add(&ws->started, 1);
    return ws->command;
}

static void worker_signal_finished(worker_state_t *ws)
{
    __sync_fetch_and_add(&ws->finished, 1);
}
...
static s32 trigger_ucred_triple_free(exploit_ctx_t *ctx)
{
    ...
    worker_init(&ctx->iov_state, IOV_THREAD_NUM);
    worker_init(&ctx->uio_state, UIO_THREAD_NUM);
    ...
    CALL(ctx, fn_netcontrol, slot, NET_CONTROL_NETEVENT_CLEAR_QUEUE,
         (u64)clear_buf, 8, 0, 0);
    LOG(ctx, "tctf: netcontrol CLEAR_QUEUE done (poops)\n");
...
static s32 aio_prepare(exploit_ctx_t *ctx, const char *title_id, aio_ctx_t *aio)
{
    ...
    if (!find_file(ctx, title_id, "elfldr_1320_v5.elf", elfldr_path)) {
    ...
    if (usb_fd >= 0) {
        ...
        if (!find_file(ctx, title_id, "kexp_v6.bin", bin_path)) {
